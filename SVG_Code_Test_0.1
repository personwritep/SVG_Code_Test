...
// @version        0.1
// @description        SVG画像コードの表示テストをする 「Scroll Lock」
// @author        Ameba Blog User
// @match        https://ameblo.jp/*
// @icon        https://www.google.com/s2/favicons?sz=64&domain=ameblo.jp
// @noframes
// @grant        none
// @updateURL        https://github.com/personwritep/SVG_Code_Test/raw/main/SVG_Code_Test.user.js
// @downloadURL        https://github.com/personwritep/SVG_Code_Test/raw/main/SVG_Code_Test.user.js
// ==/UserScript==
...
function main(){
    ...
    function check_key(event){
        if(event.keyCode==145){ // ショートカット「Scroll Lock」
    ...
    function text_img(code){
        ...
        if(t_img0){
            t_img0.innerHTML=''; } // 初期化

        let t_img1_style_=document.querySelector('.t_img1_style');
        if(t_img1_style_){
            t_img1_style_.remove(); } // 初期化


        let img_code=code.value; // コード取得

        get_text0(img_code); //「'」タイプコード
        let stack_str0=stack_g.join('');
        t_img0.innerHTML=stack_str0;


        get_text1(img_code); //「"」タイプコード
